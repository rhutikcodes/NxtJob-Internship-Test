const config = {
  ...
  <body>
    ...
    <a href="https://github.com/vedantbajpai/URL_Shortner/" target="_self">Fork me on GitHub</a>
  ...
  async function is_url_safe(url){
    ...
    let raw = JSON.stringify({"client":{"clientId":"URL_Shortner","clientVersion":"1.0.7"},"threatInfo":{"threatTypes":["MALWARE","SOCIAL_ENGINEERING","POTENTIALLY_HARMFUL_APPLICATION","UNWANTED_SOFTWARE"],"platformTypes":["ANY_PLATFORM"],"threatEntryTypes":["URL"],"threatEntries":[{"url":url}]}});
  ...
  async function handleRequest(request) {
    ...
    console.log(path)
    if(!path){
      const html= await fetch("https://vedantbajpai.github.io/URL_Shortner/"+config.theme+"/index.html")
    ...
    if (location) {
      if (config.safe_browsing_api_key){
        if(!(await is_url_safe(location))){
          let warning_page = await fetch("https://vedantbajpai.github.io/URL_Shortner/safe-browsing.html")
      ...
      if (config.no_ref=="on"){
        let no_ref= await fetch("https://vedantbajpai.github.io/URL_Shortner/no-ref.html")
